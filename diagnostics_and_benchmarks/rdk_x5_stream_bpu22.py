...
def main():
    ...
    parser.add_argument("--bpu-model", default=os.path.expanduser(
                        "~/rdk_model_zoo/samples/vision/ultralytics_yolo/model/"
                        "yolo11n_detect_bayese_640x640_nv12.bin"),
                        help="Path to the .bin model")
    parser.add_argument("--bpu-classes", type=int, default=80,
                        help="80 for the stock COCO model, 1 for a converted "
                             "single-class model")
    parser.add_argument("--bpu-keep-class", type=int, default=0,
                        help="COCO class to keep; 0 is person. -1 keeps all, "
                             "which is what a single-class model needs.")
    ...
    keep = None if args.bpu_keep_class < 0 else args.bpu_keep_class

    thm.infer = InferThread(
        "Thermal", thm.infer_in,
        args.bpu_model if args.thermal_backend == "bpu" else thm_model,
        args.imgsz, args.conf, args.threads,
        backend=args.thermal_backend,
        classes_num=args.bpu_classes, keep_class=keep)
    rgb.infer = InferThread(
        "RGB", rgb.infer_in,
        args.bpu_model if args.rgb_backend == "bpu" else rgb_model,
