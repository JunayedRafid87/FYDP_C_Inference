...
#     "T/RH: 30.3C  78%"
# Generic key:value matching reads "RH: 30.3" out of that and files the
# TEMPERATURE as humidity, dropping the real humidity entirely. This line
...
# Parsed naively it sets co=0.124, ch4=0.938 — plausible-looking garbage.
...
def parse_line(line):
    """Prefer the '#J {...}' JSON line; fall back to the human-readable text.

    Once a JSON line has been seen, loose parsing is disabled permanently —
    the two formats disagree about units in places, and JSON is the one that
    was designed to be read by a machine.
    """
    global SEEN_JSON
