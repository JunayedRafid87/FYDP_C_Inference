#!/usr/bin/env python3
"""
rover_cams.py — ground station page + both live camera feeds.

    RGB     : IMX219 NoIR, MIPI CSI, NV12, JPEG
    Thermal : UVC module, USB, YUYV 256x192, WHITE-HOT greyscale, LOSSLESS PNG

Thermal is sent as PNG, not JPEG. At 256x192 greyscale a lossless frame is
only ~40 KB, so compression buys little and costs detail the model needs.
It stays greyscale white-hot to match the training data for
thermal_yolo11n_v3 — false colour would feed it relationships it never saw.

Live controls at /config (max fps and sharpness, per feed) are wired to the
sliders in the VISION panel. Capping fps below the sensor rate is the main
CPU lever: frames above the cap are read and discarded without encoding.

Put Rover_Ground_Station.html next to this script and it gets served at /.
Serving the page from here keeps feeds same-origin — absolute http:// URLs
get silently upgraded to https:// when the page loads from an https
context, and the resulting TLS handshake shows up as 'Bad request syntax'.
...
# Live-adjustable. Sharpness is unsharp-mask amount, 0 = off.
SETTINGS = {
