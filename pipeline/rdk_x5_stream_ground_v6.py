...
# ---------------------------------------------------------------------------
# System telemetry: per-core CPU load and clocks, BPU load/clock, temps, RAM.
#
# Read from /proc and sysfs where possible (no root, no subprocess), falling
# back to hrut_somstatus for the Horizon-specific BPU counters. hrut_somstatus
# usually needs root, so if the server is not running as root those fields are
...
class PerfMonitor(threading.Thread):
    """Samples system state once a second and publishes it for /perf."""
    ...
    def _cpu_clocks(self):
        ...
        for i in range(self.ncpu):
            khz = _read(f"/sys/devices/system/cpu/cpu{i}/cpufreq/scaling_cur_freq",
                        int, None)
            if khz:
    ...
    # ---- Horizon-specific: BPU load, BPU clock, named temps -------------
    def _somstatus(self):
        """Parse hrut_somstatus. Needs root; returns {} if unavailable."""
        try:
            r = subprocess.run(["hrut_somstatus"], capture_output=True,
                               text=True, timeout=3)
            txt = r.stdout
        except Exception:
            return {}
        ...
        out = {"temps": {}, "cpu_clock": {}, "bpu": {}}
        for m in re.finditer(r"(DDR|BPU|CPU)\s*:\s*([\d.]+)\s*\(C\)", txt):
            out["temps"][m.group(1)] = float(m.group(2))
        for m in re.finditer(r"cpu(\d+):\s*(\d+)\s+(\d+)\s+(\d+)", txt):
            out["cpu_clock"][f"cpu{m.group(1)}"] = int(m.group(3))
        m = re.search(r"bpu(\d+):\s*(\d+)\s+(\d+)\s+(\d+)\s+(\d+)", txt)
        if m:
            out["bpu"] = {"cur": int(m.group(3)), "max": int(m.group(4)),
                          "ratio": int(m.group(5))}
        return out
    ...
    def _mem(self):
        ...
        return {"total_mb": round(total / 1024), "used_mb": round((total - avail) / 1024),
                "pct": round(100.0 * (total - avail) / total, 1) if total else 0.0}

    def run(self):
        self._cpu_snapshot()          # prime the delta
        while self.running:
            time.sleep(self.interval)
            loads = self._cpu_loads()
            clocks = self._cpu_clocks()
            som = self._somstatus()
            if som.get("cpu_clock"):
                clocks.update(som["cpu_clock"])

            cores = []
            for i in range(self.ncpu):
                k = f"cpu{i}"
                cores.append({"i": i, "load": loads.get(k, 0.0),
                              "clock": clocks.get(k, 0)})

            temps = dict(self._thermal_zones())
            temps.update(som.get("temps", {}))
            ...
            with PERF_LOCK:
                ...
                PERF.update({
                    "cores": cores,
                    "cpu_avg": round(sum(c["load"] for c in cores) / max(len(cores), 1), 1),
                    "temps": temps,
                    "bpu": som.get("bpu", {}),
                    "mem": self._mem(),
                    "load_avg": [round(x, 2) for x in la],
                    "uptime_s": int(_read("/proc/uptime", lambda x: float(x.split()[0]), 0)),
                    "root": os.geteuid() == 0,
                    "ts": time.time(),
                })
...
class CameraWorker(threading.Thread):
    ...
    def run(self):
        ...
        while self.running:
            ...
            # Published for the browser overlay. Display-space coordinates,
            # so the client only scales by its own element size.
            ...
            if not self.draw:
                # Clean video path for teleop: no full-frame copy, no
                # rectangles, no HUD. The copy alone was ~920KB per frame at
                # 640x480 and sat in the latency path.
...
img{display:block;border-radius:8px;border:1px solid #334155;
image-rendering:auto;width:640px;height:auto}
.l{font-size:11px;letter-spacing:.12em;color:#64748b;margin-top:8px}
.s{font:12px ui-monospace,monospace;color:#94a3b8;margin-top:4px}
</style></head><body>
<h1>RDK X5 Dual-Modality Human Detection</h1>
<p>Independent streams &middot; detection asynchronous</p>
<div class="g">
  <div class="p"><img src="/thermal_feed"><div class="l">THERMAL</div>
    <div class="s" id="sThermal">&mdash;</div></div>
  <div class="p"><img src="/rgb_feed"><div class="l">RGB / NoIR</div>
    <div class="s" id="sRGB">&mdash;</div></div>
</div>
<script>
setInterval(function(){
  fetch('/stats').then(r=>r.json()).then(d=>{
    for (const k of ['Thermal','RGB']) {
      const s = d[k], el = document.getElementById('s'+k);
      if (s && el) el.textContent =
        s.w+'x'+s.h+' \u00b7 '+s.fps+' fps \u00b7 det '+s.det_hz+' Hz ('+
        s.infer_ms+' ms) \u00b7 '+s.n+' det';
    }
  }).catch(()=>{});
}, 1000);
</script>
...
def main():
    ...
    parser.add_argument("--thermal-conf", type=float, default=None,
                        help="Confidence override for thermal. The measured "
                             "F1-optimal point is 0.45; 0.50-0.55 trades ~0.2%% "
                             "recall to suppress hands and other isolated hot "
                             "blobs read as people.")
    parser.add_argument("--rgb-conf", type=float, default=None,
                        help="Confidence override for RGB.")
    parser.add_argument("--overlay", action="store_true",
                        help="Stream video untouched and let the browser draw "
                             "boxes from /detections. Removes a full-frame copy "
                             "and all drawing from the video path.")
    parser.add_argument("--no-perf", action="store_true",
                        help="Skip system telemetry sampling")
    ...
    thm.infer = InferThread(
        ...
        args.imgsz,
        args.thermal_conf if args.thermal_conf is not None else args.conf,
        args.threads,
    ...
    rgb.infer = InferThread(
        ...
        rgb_sz,
        args.rgb_conf if args.rgb_conf is not None else args.conf,
        args.threads,
