...
class InferThread(threading.Thread):
    ...
    def __init__(self, name, src_slot, model_path, imgsz, conf, threads,
                 backend="onnx", classes_num=80, keep_class=0, iou=0.45):
        super().__init__(daemon=True)
...
def main():
    ...
    parser.add_argument("--conf", type=float, default=0.40,
                        help="Default confidence for both models")
    parser.add_argument("--thermal-conf", type=float, default=None,
                        help="Override for thermal. Your conf_sweep gives "
                             "0.45 as F1-optimal and 0.50-0.55 for higher "
                             "precision; raise it to suppress hands and other "
                             "isolated hot blobs read as people.")
    parser.add_argument("--rgb-conf", type=float, default=None,
                        help="Override for RGB.")
    parser.add_argument("--iou", type=float, default=0.45,
                        help="NMS overlap threshold. Lower merges nearby "
                             "boxes more aggressively; raise it if two people "
                             "standing close get merged into one.")
    ...
    thm.infer = InferThread(
        ...
        args.imgsz, args.thermal_conf if args.thermal_conf is not None else args.conf,
        args.threads,
        backend=args.thermal_backend,
        iou=args.iou, classes_num=args.thermal_bpu_classes,
    ...
    rgb.infer = InferThread(
        ...
        rgb_sz, args.rgb_conf if args.rgb_conf is not None else args.conf,
        args.threads,
        backend=args.rgb_backend,
        iou=args.iou, classes_num=args.rgb_bpu_classes,
